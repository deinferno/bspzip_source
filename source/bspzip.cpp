...
{
	FileSystem_Init( pFilename, maxMemoryUsage, FS_INIT_COMPATIBILITY_MODE );
	if ( !g_pFileSystem )
...
void Usage( void )
{
	...
	fprintf( stderr, "bspzip -deletelist <bspfile> <listfile> <newbspfile>\n");
...
int main( int argc, char **argv )
{
	...
	if( ( stricmp( pAction, "-extract" ) == 0 ) && nActionArgs == 2 )
	{
		...
		CmdLib_InitFileSystemCompability( pActionArgs[0] );
	...
	else if( ( stricmp( pAction, "-deletelist" ) == 0 ) && nActionArgs == 3 )
	{
		// bspzip -deletelist <bspfile> <listfile> <newbspfile>
		CmdLib_InitFileSystemCompability( pActionArgs[0] );
	...
	else if( ( stricmp( pAction, "-repack" ) == 0 ) && ( nActionArgs == 1 || nActionArgs == 2 ) )
	{
		...
		CmdLib_InitFileSystemCompability( pFile );
